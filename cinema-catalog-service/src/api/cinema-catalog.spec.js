...
describe('Testing Cinemas API', () => {
    ...
    let testRepo = {
        ...
        getCinemaSchedule(cinemaId, movieId) {
            console.log(cinemaId, movieId)
            return Promise.resolve(testSchedulesCinema)
        }
    }
    ...
    it('can return movies by location', (done) => {
        const location = {
    ...
    it('can get movie premiers by movie id', (done) => {
        request(app)
    ...
    it('can get schedules by city and movie', (done) => {
        request(app)
            .get('/cinemas/588ababf2d029a6d15d0b5bf/1')
            .expect((res) => {
                res.body.should.containEql(testSchedulesCinema[0])
                res.body.should.containEql(testSchedulesCinema[1])
            })
            .expect(200, done)
    })
})